...
export class TransectionsService {
  constructor(
    ...
    private _transectionsRepo: Repository<Transections>,
    private _userService: UserService
  ) {}

  async getWalletHistory(filter: GetTransactionHistoryDto) {
    const { wallet_id, user_id, transection_type, status, page = 1, limit = 10, role } = filter;

    const take = Number(limit);
    const skip = (Number(page) - 1) * take;
    console.log(role);
    if (role === UserRoles.ADMIN) {
      const qb = this._transectionsRepo
  ...
  async findAll(
    ...
    message: "transection retrived successfully!";
    statusCode: 200;
  }> {
    const [data, total] = await this._transectionsRepo.findAndCount({
  ...
  async getMonthlyEarnings(
    year: number
  ): Promise<{ total: any; data: { month: string; totalEarnings: number }[] }> {
    const result = await this._transectionsRepo
      .createQueryBuilder("transection")
    ...
    const totalAmount = await this._transectionsRepo.query(
      `
  ...
  async getStatistics() {
    ...
    const users = await this._userService.getTotalUsersCount();
