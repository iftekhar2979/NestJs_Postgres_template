...
export class ShipmentService {
  ...
  async callTransglobalApi({
    ...
    packageInfo: { Weight: number; Length: number; Width: number; Height: number };
  }) {
    ...
    try {
      const response = await lastValueFrom(this.httpService.post(transglobalApiUrl, requestData));

      if (response.data?.Status === "SUCCESS") {
        return response.data;
      } else {
        console.log(response.data);
        // Throw only the notification message
        throw new Error(response.data?.Notifications || "Unknown error from Transglobal API");
      }
    } catch (error) {
      console.error("Error calling Transglobal API:", JSON.stringify(error));

      // Handle both API and Axios errors
      const errorMessage =
        error?.response?.data?.Notifications || // If Axios got a response with Notifications
        error?.message || // If you manually threw Error(Notifications)
        "Failed to call Transglobal API"; // Fallback
      let msg = "";
      console.log(errorMessage);
      for (const i of errorMessage) {
        console.log(i);
        msg = i.Message;
      }
  ...
  async transglobalFinalShipment({ QuoteId, ServiceId }: { QuoteId: number; ServiceId: number }) {
    ...
    try {
      ...
      if (response.data && response.data.Status === "SUCCESS") {
        ...
        throw new BadRequestException(response.data.Notifications);
      }
    } catch (error) {
      console.log("Here Is Error");
      console.error("Error calling Transglobal API:", error);
      throw new BadRequestException(error.Notifications);
    }
  }
  ...
  async updateOrderInfo({
    ...
    try {
      ...
      try {
        ...
        const totalAmount =
          Number(existingOrder.total) + Number(existingOrder.protectionFee) + deliveryCharge;
  ...
  async UpdateShipmentInformationLater({ user, product_id }: { user: User; product_id: number }) {
    ...
    const shipmentInfo = await this.shipmentRepository.findOne({
      where: { order: { id: existingOrder.id } },
    });
    if (shipmentInfo) {
